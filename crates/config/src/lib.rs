...
impl ConfigToml {
    ...
    pub fn merge_project_overrides(&mut self, project: ConfigToml) {
        ...
        merge_project_provider_config(&mut self.providers.xiaomi, &project.providers.xiaomi);
    }
    ...
    pub fn set_value(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "providers.xiaomi.api_key" => {
                self.providers.xiaomi.api_key = Some(value.to_string());
            }
    ...
    fn load() -> Self {
        Self {
            ...
            ollama_base_url: std::env::var("OLLAMA_BASE_URL")
                .ok()
                .filter(|v| !v.trim().is_empty()),
            xiaomi_base_url: std::env::var("MIMO_BASE_URL")
                .ok()
                .filter(|v| !v.trim().is_empty()),
        }
    }
