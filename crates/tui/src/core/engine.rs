...
impl Engine {
    fn mode_runtime_instructions(mode: AppMode) -> &'static str {
        mode::mode_runtime_instructions(mode)
    }
...
// outside messages[0].
#[cfg(test)]
use self::trust::agent_approval_mode_for_turn;
use self::trust::{
    AutoReviewPlanDecision, ToolAskRuleDecision, auto_review_plan_decision,
    auto_review_run_origin_for_plan, effective_input_policy, exec_shell_ask_rule_decision,
...
use self::mode::{sandbox_policy_for_mode, shell_policy_for_mode};
#[cfg(test)]
