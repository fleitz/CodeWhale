...
| Sidebar sub-agent rows | sidebar unit widths 24, 40 | `sidebar::tests::agents_panel_keeps_cjk_names_and_objectives_width_safe` | CJK agent names, mixed English/CJK objectives, status/progress, and branch details stay display-width safe with full hover text |
...
- Full sub-agent/Fleet progress overlay screenshots remain under #3480 because the sidebar unit fixture now covers CJK row width, but it does not yet render a complete narrow terminal shell.
